...
-- ## 2.
SELECT p_name FROM part WHERE p_retailprice < (SELECT AVG(c_acctbal) FROM customer);
SELECT p_name FROM part GROUP BY p_name HAVING AVG(p_retailprice) < (SELECT AVG(c_acctbal) FROM customer);
...
-- ## 4.
SELECT l_comment FROM lineitem WHERE l_quantity = ANY (SELECT p.p_size FROM part AS p, lineitem AS l WHERE p.p_partkey=l.l_partkey AND p_type='LARGE POLISHED COPPER' AND l.l_shipdate BETWEEN '1998-07-27' AND '1998-12-31');
